...
function btnClick_ReadTweat(id, title){
    $.ajax({
        ...
        url: '/home_release_update/' + id
          ...
          url = 'https://homisoft.com/top_tweat/' + id;
          window.location.href='https://twitter.com/intent/tweet?text=' + title + ' ' + url;
...
$(function($) {
    ...
    function home_change(){
        jQuery('.comparison_group').each(function(i){
            ...
            if(category.indexOf($('#serach_category').val()) > -1 && video_type.indexOf($('#serach_video_type').val()) > -1){
                jQuery(this).show();
            }else{
                jQuery(this).hide();
            }
